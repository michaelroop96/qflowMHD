...
def solve(W,theta, qstepsize=0.1, steps=None, qtime=None, time=None,
    ...
    for k in range(0, steps, inner_steps):
        ...
        W,theta=method(W,theta, qstepsize, steps=no_steps, **method_kwargs)
        qt += no_steps*qstepsize
        if progress_bar:
            pbar.update(no_steps)
        if callback is not None:
            for cfun in callback:
                cfun(W,theta, qt, **kwargs)
        print("Iteration {} finished. ".format(count))
        count+=1
    # Close progressbar
    if progress_bar:
        pbar.close()
