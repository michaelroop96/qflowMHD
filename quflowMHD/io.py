...
class QuData(object):
        ...
        try:
            ...
            try:
                ...
                if datapath+'W_cache' in f \
                        and datapath+'Theta_cache' in f \
                        and datapath+'qtime_cache' in f \
                        and f[datapath+'Theta_cache'].shape[0] == cache_size \
                        and f[datapath+'W_cache'].shape[0] == cache_size:
                    attrs['W_cache'] = f[datapath+'W_cache'][...]
                    attrs['Theta_cache'] = f[datapath+'Theta_cache'][...]
    ...
    def __call__(self, W,theta, qtime=0.0, **kwargs):
        ...
        if self.cache_size == 1:
            ...
            self.Theta_cache = np.array([theta])
            ...
            if self.W_cache is None:
                ...
                self.Theta_cache = np.zeros((self.cache_size,) + theta.shape, dtype=theta.dtype)
                self.qtime_cache = np.zeros(self.cache_size, dtype=float)
                self.cache_steps = 0
            self.W_cache[self.cache_steps, :, :] = W
            self.Theta_cache[self.cache_steps, :, :] = theta
    ...
    def flush(self):
        ...
        if self.cache_steps != 0:
            save(self.filename, self.W_cache[:self.cache_steps], self.Theta_cache[:self.cache_steps],qtime=self.qtime_cache[:self.cache_steps],
    ...
    def _save_attrs(self, save_cache=False):
        with h5py.File(self.filename, "a") as f:
            ...
            if save_cache:
                ...
                if self.datapath+'Theta_cache' not in f:
                    f.create_dataset(self.datapath+'Theta_cache', self.Theta_cache.shape, dtype=self.Theta_cache.dtype)
                ...
                f[self.datapath+'Theta_cache'][...] = self.Theta_cache

            f[self.datapath].attrs.update(attrs)


def save(filename, data, data1, qtime=None, qstepsize=None, N=None, qtype="shr", datapath="/", attrs=None):
    """
        ...
        if qtype == "shr" or qtype == "shc":
            ...
            if data_qtype == "shr" or data_qtype == "shc":
                ...
                for d in data1:
                    if qtype == "shc":
                        if d.ndim == 2:
                            omega1.append(mat2shc(d))
                        else:
                            raise ValueError("Something wrong with the input data.")
                    else:
                        omega1.append(as_shr(d))
            ...
            statepath = datapath + "stateW"
            statepath1 = datapath + "stateTheta"
            ...
            if statepath1 not in f:
                stateset1 = f.create_dataset(statepath1, (0, omega1.shape[-1]),
                                            dtype=omega1.dtype,
                                            maxshape=(None, omega1.shape[-1]),
                                            chunks=(1, omega1.shape[-1])
                                            )
                if N is not None:
                    assert N == round(np.sqrt(omega1.shape[-1]))
                else:
                    N = round(np.sqrt(omega1.shape[-1]))
                f[datapath].attrs["N"] = N
                f[datapath].attrs["version"] = __version__
                f[datapath].attrs["created"] = datetime.datetime.now().isoformat()
                stateset.attrs["qtype"] = qtype
            elif f[statepath].shape[-1] != omega1.shape[-1] or f[statepath].ndim != 2:
                raise ValueError("The file qtype does not seem to be correct.")
            ...
            f[statepath1].resize(f[statepath1].shape[0]+omega1.shape[0], axis=0)
            ...
            f[statepath1][-omega1.shape[0]:, :] = omega1
            ...
            f[statepath1].attrs["modified"] = modtime.isoformat()
